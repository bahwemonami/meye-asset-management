  ...
  template: `
    ...
    <section class="relative py-32 bg-gradient-to-br from-primary-950 via-primary-900 to-primary-800 overflow-hidden">
      ...
      <div class="container-custom relative z-10">
        <div class="max-w-3xl">
          <span class="inline-block text-accent-400 font-semibold text-sm tracking-wider uppercase mb-4 animate-fade-in">Our Team</span>
          <h1 class="text-white mb-6 animate-fade-in-up">Meet the <span class="text-accent-400">Experts</span></h1>
    ...
    <section class="section-padding bg-white">
      <div class="container-custom">
        <div class="text-center mb-16 reveal">
          <span class="inline-block text-accent-500 font-semibold text-sm tracking-wider uppercase mb-4">Leadership</span>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          @for (member of leadership; track member.id; let i = $index) {
            <div class="card overflow-hidden group reveal" [style.animation-delay.ms]="i * 100">
              <div class="relative aspect-[4/5] overflow-hidden">
                ...
                <div class="absolute bottom-0 left-0 right-0 p-6 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                  <div class="flex space-x-3">
                    @if (member.linkedin) {
                      <a [href]="member.linkedin" target="_blank" class="w-10 h-10 rounded-lg bg-white/20 backdrop-blur-sm flex items-center justify-center text-white hover:bg-white/30 transition-colors">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                        </svg>
                      </a>
                    }
                    <button (click)="openMemberModal(member)" class="w-10 h-10 rounded-lg bg-white/20 backdrop-blur-sm flex items-center justify-center text-white hover:bg-white/30 transition-colors">
              ...
              <div class="p-6">
                <h3 class="text-xl font-semibold text-primary-900 mb-1">{{ member.name }}</h3>
                <p class="text-accent-600 font-medium text-sm mb-3">{{ member.title }}</p>
                <div class="flex flex-wrap gap-2">
                  @for (skill of member.expertise.slice(0, 3); track skill) {
                    <span class="px-2 py-1 bg-primary-50 text-primary-700 text-xs rounded">{{ skill }}</span>
                  }
                </div>
              </div>
            </div>
    ...
    <section class="section-padding bg-primary-50/50">
      <div class="container-custom">
        <div class="text-center mb-16 reveal">
          <span class="inline-block text-accent-500 font-semibold text-sm tracking-wider uppercase mb-4">Specialists</span>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          @for (member of investmentTeam; track member.id; let i = $index) {
            <div class="card-bordered p-6 text-center group hover:-translate-y-1 transition-all duration-300 reveal" 
                 [style.animation-delay.ms]="i * 100">
              <div class="w-24 h-24 rounded-full overflow-hidden mx-auto mb-4 ring-4 ring-primary-100 group-hover:ring-accent-200 transition-all duration-300">
                <img [src]="member.image" [alt]="member.name" class="w-full h-full object-cover">
              </div>
              <h4 class="font-semibold text-primary-900 mb-1">{{ member.name }}</h4>
              <p class="text-dark-500 text-sm mb-3">{{ member.title }}</p>
              <button (click)="openMemberModal(member)" class="text-accent-600 text-sm font-medium hover:text-accent-700 transition-colors">
                View Profile →
              </button>
            </div>
          }
    ...
    <section class="section-padding bg-gradient-to-br from-primary-900 to-primary-950">
      <div class="container-custom">
        <div class="grid lg:grid-cols-2 gap-16 items-center">
          <div class="reveal">
            <span class="inline-block text-accent-400 font-semibold text-sm tracking-wider uppercase mb-4">Careers</span>
            <h2 class="text-white mb-6">Join Our <span class="text-accent-400">Team</span></h2>
            ...
            <ul class="space-y-4 mb-8">
              @for (benefit of careerBenefits; track benefit) {
                <li class="flex items-center space-x-3">
                  <div class="w-6 h-6 rounded-full bg-accent-500/20 flex items-center justify-center">
                    <svg class="w-4 h-4 text-accent-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                    </svg>
                  </div>
                  <span class="text-primary-100">{{ benefit }}</span>
                </li>
              }
            </ul>
    ...
    <!-- Member Modal -->
    @if (selectedMember()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" (click)="closeMemberModal()">
        <div class="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl" (click)="$event.stopPropagation()">
          <div class="relative">
            <img [src]="selectedMember()!.image" [alt]="selectedMember()!.name" class="w-full h-64 object-cover">
            <button (click)="closeMemberModal()" class="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/90 flex items-center justify-center hover:bg-white transition-colors">
          ...
          <div class="p-8">
            <h3 class="text-2xl font-semibold text-primary-900 mb-1">{{ selectedMember()!.name }}</h3>
            <p class="text-accent-600 font-medium mb-4">{{ selectedMember()!.title }}</p>
            <p class="text-dark-600 mb-6">{{ selectedMember()!.bio }}</p>
            <div class="mb-6">
              <h4 class="font-semibold text-primary-900 mb-3">Areas of Expertise</h4>
              <div class="flex flex-wrap gap-2">
                @for (skill of selectedMember()!.expertise; track skill) {
                  <span class="px-3 py-1 bg-primary-50 text-primary-700 text-sm rounded-full">{{ skill }}</span>
                }
              </div>
            </div>
