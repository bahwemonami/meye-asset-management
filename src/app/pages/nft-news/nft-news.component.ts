  ...
  template: `
    ...
    <section class="relative py-24 xs:py-28 sm:py-32 bg-gradient-to-br from-primary-950 via-primary-900 to-primary-800 overflow-hidden">
      ...
      <div class="container-custom relative z-10">
        <div class="max-w-3xl">
          <a routerLink="/digital-proof" class="inline-flex items-center text-primary-300 hover:text-accent-400 transition-colors mb-4 xs:mb-6 text-sm xs:text-base">
            <svg class="w-3.5 h-3.5 xs:w-4 xs:h-4 mr-1.5 xs:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
            </svg>
            Back to Digital Proof
          </a>
          <h1 class="text-white mb-4 xs:mb-6 animate-fade-in-up">News & <span class="text-accent-400">Updates</span></h1>
    ...
    <!-- Filter Section -->
    <section class="py-4 xs:py-6 md:py-8 bg-white border-b border-primary-100 sticky top-16 xs:top-18 sm:top-20 z-40">
      <div class="container-custom">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 xs:gap-4">
          <!-- Category Filters -->
          <div class="flex flex-wrap gap-1.5 xs:gap-2">
            @for (category of categories; track category.value) {
              <button 
                (click)="setActiveCategory(category.value)"
                class="px-2.5 py-1.5 xs:px-4 xs:py-2 rounded-lg text-xs xs:text-sm font-medium transition-all duration-300"
          ...
          <div class="relative">
            <input 
              ...
              [(ngModel)]="searchQuery"
              class="w-full md:w-56 lg:w-64 pl-8 xs:pl-10 pr-3 xs:pr-4 py-1.5 xs:py-2 border border-primary-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-300 text-sm">
            <svg class="absolute left-2.5 xs:left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 xs:w-4 xs:h-4 text-dark-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if (featuredArticle(); as featured) {
      <section class="section-padding bg-primary-50/30">
        <div class="container-custom">
          <div class="grid lg:grid-cols-2 gap-6 xs:gap-8 items-center">
            <div class="relative aspect-video lg:aspect-square rounded-xl xs:rounded-2xl overflow-hidden reveal">
              ...
              <span class="absolute top-3 left-3 xs:top-4 xs:left-4 px-2 py-0.5 xs:px-3 xs:py-1 bg-accent-500 text-white text-xs font-medium rounded-full">Featured</span>
            </div>
            <div class="reveal-right">
              <span class="inline-block px-2 py-0.5 xs:px-3 xs:py-1 bg-primary-100 text-primary-700 text-xs xs:text-sm font-medium rounded-full mb-3 xs:mb-4">
                {{ featured.category }}
              </span>
              <h2 class="text-xl xs:text-2xl md:text-3xl font-semibold text-primary-900 mb-3 xs:mb-4">{{ featured.title }}</h2>
              <p class="text-dark-600 text-sm xs:text-base mb-4 xs:mb-6">{{ featured.excerpt }}</p>
              <div class="flex items-center text-xs xs:text-sm text-dark-500 mb-4 xs:mb-6">
                <span>{{ featured.date }}</span>
                <span class="mx-2 xs:mx-3">•</span>
                <span>{{ featured.readTime }} read</span>
              </div>
              <a href="#" class="btn-primary">
                Read Article
                <svg class="w-3.5 h-3.5 xs:w-4 xs:h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <section class="section-padding bg-white">
      <div class="container-custom">
        @if (filteredArticles().length > 0) {
            ...
            @for (article of filteredArticles(); track article.id; let i = $index) {
              <article class="card overflow-hidden group hover:-translate-y-1 transition-all duration-300 reveal" 
                ...
                <div class="p-4 xs:p-5 md:p-6">
                  <span class="inline-block px-1.5 py-0.5 xs:px-2 xs:py-1 bg-primary-50 text-primary-700 text-xxs xs:text-xs font-medium rounded mb-2 xs:mb-3">
                    {{ article.category }}
                  </span>
                  <h3 class="text-base xs:text-lg font-semibold text-primary-900 mb-1 xs:mb-2 group-hover:text-accent-600 transition-colors duration-300">
                    {{ article.title }}
                  </h3>
                  <p class="text-dark-500 text-xs xs:text-sm mb-3 xs:mb-4 line-clamp-2">{{ article.excerpt }}</p>
                  <div class="flex items-center justify-between text-xxs xs:text-sm">
          ...
          <div class="text-center py-12 xs:py-16">
            <svg class="w-12 h-12 xs:w-16 xs:h-16 text-dark-300 mx-auto mb-3 xs:mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
            <h3 class="text-lg xs:text-xl font-semibold text-dark-700 mb-1 xs:mb-2">No articles found</h3>
            <p class="text-dark-500 text-sm xs:text-base">Try adjusting your search or filter criteria.</p>
          </div>
        }
      </div>
    </section>
    ...
    <section class="section-padding bg-gradient-to-br from-primary-900 via-primary-800 to-primary-900">
      <div class="container-custom">
        <div class="max-w-2xl mx-auto text-center">
          <h2 class="text-white mb-3 xs:mb-4">Subscribe to Our <span class="text-accent-400">Newsletter</span></h2>
          <p class="text-primary-200 text-sm xs:text-base mb-6 xs:mb-8">
            Get the latest compliance updates, product news, and insights delivered directly to your inbox.
          </p>
          <form class="flex flex-col xs:flex-row gap-3 xs:gap-4 max-w-md mx-auto" (submit)="onNewsletterSubmit($event)">
            <input 
              type="email" 
              placeholder="Enter your email"
              class="flex-1 px-3 xs:px-4 py-2.5 xs:py-3 rounded-lg bg-white/10 border border-white/20 text-white placeholder-primary-400 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-transparent text-sm">
            <button type="submit" class="btn-accent !py-2.5 xs:!py-3 whitespace-nowrap">
              Subscribe
              <svg class="w-3.5 h-3.5 xs:w-4 xs:h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <p class="text-primary-400 text-xs xs:text-sm mt-3 xs:mt-4">
